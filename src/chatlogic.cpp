...
ChatLogic::~ChatLogic()
{
    ...
    // No need anymore as we are using smart pointers
    // delete all nodes
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        {
                            std::shared_ptr<GraphNode> node = std::make_shared<GraphNode>(id);
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                           ...
                           std::unique_ptr<GraphEdge> childEdge(edge);
                            (*parentNode)->AddEdgeToChildNode(std::move(childEdge));
                        }
